...
static int  gBatteryInitFcc=1520;
...
static int get_prop_init_batt_fcc(struct pm8921_chg_chip *chip)
{
	int rc;
...
void UpdateGlobalValue(void)
{
	...
	get_prop_init_batt_fcc(the_chip);
	pr_debug("[BAT][8921]%s()---\n",__FUNCTION__);
}
//ASUS BSP --- Eason Chang add BAT global variable
...
#if 0
		val->intval = get_prop_batt_init_fcc();
		if (val->intval < 0)
			return val->intval;
		else
			val->intval *= 1000;
#else
		val->intval = gBatteryInitFcc;
		pr_debug("POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN val=%d\r\n",val->intval);
#endif
		break;
