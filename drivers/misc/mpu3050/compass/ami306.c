...
int ami306_file_read(char * file_name, unsigned char *buf, int read_len)
{
    int ret=0;
    struct file *fp;
    mm_segment_t oldfs;
...
int ami306_file_write(char * file_name, unsigned char *buf, int len)
{
    int ret=0;
    struct file *fp;
    mm_segment_t oldfs;
...
int ami306_recover_compass_calibration_file(void)
{
    int ret = 0;
    ...
    if(!ami306_file_read(ASUS_COMPASS_CALIBRATION_FILE,cali_buf,256) && !ret)
    {
            ...
            if(0 != ami306_file_write(AMI_CALIBRATION_FILE,temp_buf,strlen(temp_buf)))
            {
                ret = 2;
            ...
            if(0 != ami306_file_write(ASUS_PAD_CALIBRATION_FILE,temp_buf,strlen(temp_buf)))
            {
                ret = 3;
            ...
            memcpy(temp_buf+7,cali_buf+8, 151);
            strcat(temp_buf, AMI_INTER_OFFSET_PHONE);
            if(0 != ami306_file_write(ASUS_PHONE_CALIBRATION_FILE,temp_buf,strlen(temp_buf)))
            {
                ret = 4;
...
int Check_and_recover_Calibration_file(void)
{
    int ret = 0;
    int mem_index;
    int count = 0;
    ...
    if(0 == ami306_file_read(AMI_CALIBRATION_FILE,temp_buf,256))
    {
            //parser inter offset
...
//ASUS_BSP +++ Jason Chang "check and recover compass calibration file when compass resume"
        if(Check_and_recover_Calibration_file())
        {
            ami306_recover_compass_calibration_file();
        }
//ASUS_BSP --- Jason Chang "check and recover compass calibration file when compass resume"
...
static ssize_t read_compass_raw(struct device *dev, struct device_attribute *devattr, char *buf)
{
    ...
    if(compass->compass_status)
    {
        ...
        inv_serial_single_write(client->adapter, compass->pdata->address,0x1B, 0x92);
        inv_serial_single_write(client->adapter, compass->pdata->address,0x1D, 0x40);

        res = inv_serial_read(client->adapter, compass->pdata->address,0x10, 6, data);
        if(res){
