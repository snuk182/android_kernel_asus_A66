...
#define MPUIRQ_NAME "mpuirq_6050"

/* function which gets accel data and sends it to MPU */

DECLARE_WAIT_QUEUE_HEAD(mpuirq_wait_6050);
			   ...
			   char *buf, size_t count, loff_t *ppos)
{
	...
	if (!mpuirq_dev_data.data_ready &&
		...
		wait_event_interruptible_timeout(mpuirq_wait_6050,
...
unsigned int mpuirq_poll_6050(struct file *file, struct poll_table_struct *poll)
{
	int mask = 0;
//printk("jason: mpuirq_poll_6050+\n");
	poll_wait(file, &mpuirq_wait_6050, poll);
	if (mpuirq_dev_data.data_ready)
...
static long mpuirq_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	int retval = 0;
	int data;
//printk("jason: mpuirq_ioctl_6050+\n");
	switch (cmd) {
...
static irqreturn_t mpuirq_handler(int irq, void *dev_id)
{
	...
	wake_up_interruptible(&mpuirq_wait_6050);
...
const struct file_operations mpuirq_fops_6050 = {
	.owner = THIS_MODULE,
	.read = mpuirq_read,
	.poll = mpuirq_poll_6050,
...
static struct miscdevice mpuirq_device = {
	...
	.name = MPUIRQ_NAME,
	.fops = &mpuirq_fops_6050,
};

int mpuirq_init_6050(struct i2c_client *mpu_client, struct mldl_cfg *mldl_cfg)
{
	int res;
