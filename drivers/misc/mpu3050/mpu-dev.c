...
static int g_ecompass_calibration_mode = 0;
//ASUS_BSP +++ Jason Chang "dynamic change orientation"
struct mldl_cfg *asus_g_mldl_cfg;
...
//A66_SR1
//A66_ER1
static signed char Pad_accel_orientation_A66sr1 [9] = { 0, 1, 0, -1, 0, 0, 0, 0, 1 };
static signed char Phone_accel_orientation_A66sr1 [9] = { -1, 0, 0, 0, -1, 0, 0, 0, 1 };
...
struct delayed_work pad_plugin_work;
struct delayed_work pad_plugout_work;
struct workqueue_struct *pad_plug_wq = NULL;
int g_calibration_file_flag = 0;
//ASUS_BSP --- Jason Chang "replace compass calibration by workqueue"
...
int SetCalibrationMode(int mode)
{
    int ret = 0;
    g_ecompass_calibration_mode = mode;
    if(g_ecompass_calibration_mode)
        printk("[mpu] In calibration mode\n");
    else
        printk("[mpu] normal mode\n");

    if(g_ecompass_calibration_mode != mode)
        ret = 1;
    return ret;
}
//ASUS_BSP --- Jason Chang "Ignore read g sensor when calibration"
...
int mpu_file_read(char * file_name, unsigned char *buf, int read_len)
{
    int ret=0;
    struct file *fp;
    mm_segment_t oldfs;
...
int mpu_file_write(char * file_name, unsigned char *buf, int len)
{
    int ret=0;
    struct file *fp;
    mm_segment_t oldfs;
...
int Create_compass_calibration_file(void)
{
    int ret = 0;
    ...
    if(!mpu_file_read(ASUS_COMPASS_CALIBRATION_FILE,cali_buf,256) && !ret)
    {
            ...
            if(0 != mpu_file_write(AMI_CALIBRATION_FILE,temp_buf,strlen(temp_buf)))
            {
                ret = 2;
            ...
            if(0 != mpu_file_write(ASUS_PAD_CALIBRATION_FILE,temp_buf,strlen(temp_buf)))
            {
                ret = 3;
            ...
            memcpy(temp_buf+7,cali_buf+8, 151);
            strcat(temp_buf, AMI_INTER_OFFSET_PHONE);
            if(0 != mpu_file_write(ASUS_PHONE_CALIBRATION_FILE,temp_buf,strlen(temp_buf)))
            {
                ret = 4;
...
//ASUS_BSP --- Jason Chang "Recover compass calibration file when file crashed"
//ASUS_BSP +++ Jason Chang "replace compass calibration by workqueue"
int mpu_copy_calibration_file(char *target_name, char *source_name)
{
    int ret=0;
    int debug = 0;
...
int asus_switch_compass_calibration_file(int current_status)
{
    int ret = 0;
    
    if(current_status)
    {
        ret += mpu_copy_calibration_file(ASUS_PHONE_CALIBRATION_FILE, AMI_CALIBRATION_FILE);
        ret += mpu_copy_calibration_file(AMI_CALIBRATION_FILE, ASUS_PAD_CALIBRATION_FILE);
    }
    else
    {
        ret += mpu_copy_calibration_file(ASUS_PAD_CALIBRATION_FILE, AMI_CALIBRATION_FILE);
        ret += mpu_copy_calibration_file(AMI_CALIBRATION_FILE, ASUS_PHONE_CALIBRATION_FILE);
    }
...
//ASUS_BSP +++ Jason Chang "dynamic change orientation"
void mpu_change_rotation_axes(int value) {
    
    if(value == MPU_EVENT_NOTIFY_PAD)
    {
        printk("[mpu] change ori to PAD\n");
        memcpy(asus_g_mldl_cfg->pdata->orientation, Pad_Gyro_Orient, 9);
        memcpy(asus_g_mldl_cfg->pdata_slave[EXT_SLAVE_TYPE_ACCEL]->orientation, Pad_Acc_Orient, 9);
        memcpy(asus_g_mldl_cfg->pdata_slave[EXT_SLAVE_TYPE_COMPASS]->orientation, Pad_Mag_Orient, 9);
    }
    else if(value == MPU_EVENT_NOTIFY_PHONE)
    {
        printk("[mpu] change ori to PHONE\n");
        memcpy(asus_g_mldl_cfg->pdata->orientation, Phone_Gyro_Orient, 9);
        memcpy(asus_g_mldl_cfg->pdata_slave[EXT_SLAVE_TYPE_ACCEL]->orientation, Phone_Acc_Orient, 9);
        memcpy(asus_g_mldl_cfg->pdata_slave[EXT_SLAVE_TYPE_COMPASS]->orientation, Phone_Mag_Orient, 9);
    }
    g_orientation_flag = value;

}

void mpu_get_orientation(void)
{
    switch (g_A60K_hwID)
    {
        case A60K_EVB:
...
static int mp_sensor_event(struct notifier_block *this, unsigned long event, void *ptr)
{
        ...
        case P01_ADD:
                printk("[mpu] PAD ADD \r\n");
                mpu_change_rotation_axes(MPU_EVENT_NOTIFY_PAD);
                //ASUS_BSP +++ Jason Chang "replace compass calibration by workqueue"
                cancel_delayed_work_sync(&pad_plugout_work);
                queue_delayed_work(pad_plug_wq, &pad_plugin_work, 0);
                //ASUS_BSP --- Jason Chang "replace compass calibration by workqueue"
                return NOTIFY_DONE;
                
        case P01_REMOVE:
                printk("[mpu] PAD REMOVE \r\n");
                mpu_change_rotation_axes(MPU_EVENT_NOTIFY_PHONE);
                //ASUS_BSP +++ Jason Chang "replace compass calibration by workqueue"
                cancel_delayed_work_sync(&pad_plugin_work);
                queue_delayed_work(pad_plug_wq, &pad_plugout_work, 0);
                //ASUS_BSP --- Jason Chang "replace compass calibration by workqueue"
                return NOTIFY_DONE;
...
{
    g_calibration_file_flag = 1;
    if(asus_switch_compass_calibration_file(MPU_EVENT_NOTIFY_PAD))
        printk("[mpu] switch calibration file to PAD failed\n");
    g_calibration_file_flag = 0;
    return;
}
...
{
    g_calibration_file_flag = 1;
    if(asus_switch_compass_calibration_file(MPU_EVENT_NOTIFY_PHONE))
...
static int mpu_dev_open(struct inode *inode, struct file *file)
{
	...
	//ASUS_BSP +++ Jason Chang "Fix open device failed"
/*	if (mpu->pid) {
		mutex_unlock(&mpu->mutex);
		return -EBUSY;
	}*/
...
//ASUS_BSP +++ Jason Chang "dynamic change orientation"
int asus_orient_status_read(void)
{
...
//ASUS_BSP --- Jason Chang "Recover compass calibration file when file crashed"
	...
	slave_adapter[EXT_SLAVE_TYPE_GYROSCOPE] = client->adapter;
//ASUS_BSP +++ Jason Chang "Ignore system request g sensor data when compass calibration"
if(cmd == MPU_READ_ACCEL)
{
        retval = copy_from_user(&magic_num,(unsigned char __user *)arg,sizeof(magic_num));
        if(g_ecompass_calibration_mode && ((magic_num[0] != 0x01) || (magic_num[1] != 0x23) || (magic_num[2] != 0x45) || (magic_num[3] != 0x67) || (magic_num[4] != 0x89) || (magic_num[5] != 0xaa)))
        {
            if ((++l_cnt % 50) == 0) 
            printk("[mpu]system request G sensor in calibration mode\n");
            msleep(20);
            ...
            magic_num[4]=0x00;
            magic_num[5]=0x04;
            err = copy_to_user((unsigned char __user *)arg, temp, slave[EXT_SLAVE_TYPE_ACCEL]->read_len);

            if (err) {
            	printk("[mpu]g sensor return fake raw data failed\n");
                retval = -EFAULT;
        	}
            return retval;
        }
}
//ASUS_BSP --- Jason Chang "Ignore system request g sensor data"
        ...
        //ASUS_BSP +++ Jason Chang "switch compass calibration data when plug in/out PAD"
            if(one_time && retry < 5)
            {
                ...
                else
                {
                    ...
                    if(IS_ERR(fp))
                    {
                        if(0 != mpu_file_read(ASUS_COMPASS_CALIBRATION_FILE,cali_buf,256))
                        {
                        ...
                        {
                            Create_compass_calibration_file();
                            one_time = 0;
                        }
                    }
                    else
                    {
                        ...
                        memset(temp_buf, 0, 256);
                        count = 0;
                        if(0 == mpu_file_read(AMI_CALIBRATION_FILE,temp_buf,256))
                        {
                            //parser inter offset
	...
	case MPU_READ_ACCEL:
//ASUS_BSP +++ Jason Chang "Ignore read g sensor when calibration"
		retval = copy_from_user(&temp,(unsigned char __user *)arg,sizeof(temp));
		if(retval)
		{
			if(g_ecompass_calibration_mode)
			{
				printk("[mpu] copy data from user failed\n");
			}
			retval = -EINVAL;
		}
		if((!g_ecompass_calibration_mode) || (g_ecompass_calibration_mode && (temp[0] == 0x01)&& (temp[1] == 0x23) && (temp[2] == 0x45) && (temp[3] == 0x67) && (temp[4] == 0x89) && (temp[5] == 0xaa)))
		{
...
/*        else if(g_ecompass_calibration_mode && ((temp[0] != 0x01) || (temp[1] != 0x23) || (temp[2] != 0x45) || (temp[3] != 0x67) || (temp[4] != 0x89) || (temp[5] != 0xaa)))
        {
			...
			if (err) {
            	printk("[mpu]copy to user failed\n");
                retval = -EFAULT;
        	}
        }*/
//ASUS_BSP --- Jason Chang "Ignore read g sensor when calibration"
...
//ASUS_BSP +++ Jason Chang "dynamic change orientation"
        case ASUS_MPU_READ_ORIENT_STASUS:
                {
                        retval = asus_orient_status_read();
                        //printk("mpu: orientation = %d\n",retval);
                }
                break;
//ASUS_BSP --- Jason Chang "dynamic change orientation"
//ASUS_BSP +++ Jason Chang "Ignore read g sensor when calibration"
        case ASUS_MPU_SET_CALIBRATION_MODE:
		{
			retval = SetCalibrationMode(arg);
		}
		break;
//ASUS_BSP --- Jason Chang "Ignore read g sensor when calibration"
//ASUS_BSP +++ Jason Chang "replace compass calibration by workqueue"
        case ASUS_CHECK_COMPASS_CALIBRATION_STATUS:
                {
                        retval = g_calibration_file_flag;
                }
                break;
//ASUS_BSP --- Jason Chang "replace compass calibration by workqueue"
	default:
...
//ASUS_BSP +++ Jason Chang "dynamic change orientation"
mpu_get_orientation();
asus_g_mldl_cfg = mldl_cfg;
//ASUS_BSP --- Jason Chang "dynamic change orientation"

//ASUS_BSP +++ Jason Chang "replace compass calibration by workqueue"
        INIT_DELAYED_WORK(&pad_plugin_work, mpu_plugin_pad_work);
        INIT_DELAYED_WORK(&pad_plugout_work, mpu_plugout_pad_work);
        if(!(pad_plug_wq = create_singlethread_workqueue("mpu_pad_plug_wq")))
            printk("[mpu] error: create pad plug workqueue failed\n");
//ASUS_BSP --- Jason Chang "replace compass calibration by workqueue"

	mldl_cfg->mpu_chip_info->addr = client->addr -1;
...
//ASUS_BSP--- Jason Chang "change oritation for pad"
//ASUS_BSP +++ Jason Chang "ATD Interface of 9-axis sensor"
        mpu_private_data->attrs.attrs = mpu_3050_attr;
	res = sysfs_create_group(&client->dev.kobj, &mpu_private_data->attrs);
	if (res) {
		dev_err(&client->dev, "Not able to create the sysfs\n");
		printk("%s:Not able to create the sysfs\n", __FUNCTION__);
	}
	mpu_private_data->gyro_status = 1;
//ASUS_BSP --- Jason Chang "ATD Interface of 9-axis sensor"
	return res;
