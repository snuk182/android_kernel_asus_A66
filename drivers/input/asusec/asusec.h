...
#define P02_ER2					1	// 0: before P02_ER1 1: after P02_ER2

/*
 * Debug Utility
 */
#define ASUSEC_FUN_LINE \
	printk(KERN_INFO "asusec : [%s]  line : [%d]\n", __FUNCTION__, __LINE__)
...
#define LID_SDEV_NAME			"asus_dock_lid"
#define CONVERSION_TIME_MS		50
#define I2C_ERR_COUNT			20

#define ASUSEC_MICROP_RETRY		10
#define ASUSEC_HUB_SLEEP_RETRY	20
#define ASUSEC_BL_RETRY			40
#define ASUSEC_RETRY_COUNT		3
#define ASUSEC_RETRY_BAT_COUNT	5
...
#define ASUSEC_LID_OPEN_UEVENT		0
#define ASUSEC_LID_CLOSE_UEVENT		1
...
#define DEBOUNCE_ERR			-1
#define DEBOUNCE_CONTINUOUS		4
...
#define ASUSEC_KEYBOARD_ENABLE	0
#define ASUSEC_KEYBOARD_DISABLE	1
...
#define ASUSEC_DISABLE_KEY	_IOR(ASUSEC_IOC_MAGIC,	8,	int)
...
struct asusec_chip {
	...
	struct wake_lock	wake_lock_keywakeup;
	struct work_struct asusec_work;
	struct delayed_work asusec_dock_init_work;
	struct delayed_work asusec_fw_update_work;
	struct delayed_work asusec_i2c_err_check_work;
	struct delayed_work asusec_led_on_work;
	struct delayed_work asusec_led_off_work;
	struct delayed_work asusec_lid_work;
	struct delayed_work asusec_resume_work;
	struct delayed_work asusec_bat_work;
	...
	char dock_pid[32];
	int polling_rate;
	int dock_in;	// 0: without dock, 1: with dock, -1: dock error
	...
	int isNeedResume;	// suspend -> resume  0: dock init successfully, 1: dock not ready, 2: init ing
	int last_bat;
	int bat_info;		// 0 : no event, 1 : ext_power_in, 2 : ext_power_out 4 : power_bad
	int pre_suspend_enable; // 0 : no early_suspend // 1 : early_suspend
	int init_retry_count;
	int i2c_err_count;
	int pad_err_count;
	int isDockHWConnected;  // 0 : MicroP : dock out / 1 : MicroP dock in / -1 : MicroP error
	int isHubSleepOn;		// 0 : Off  / 1 : on / -1 : MicroP error 
	bool UsedPowerKey; //0 : never 1: used
	bool isExtPower;
	bool isUseWakeLock;
};


// ASUS_BSP Jiunhau_Wang battery charging status report+++
#define EC_BAT_AC_MASK 		0x1
#define EC_BAT_BAT_MASK 	0x2
#define EC_BAT_C_MASK 		0x4
#define EC_BAT_F_C_MASK		0x8
// ASUS_BSP Jiunhau_Wang battery charging status report---

#define EC_EXT_POWER_PLUG_IN 	0x1
#define EC_EXT_POWER_PLUG_OUT 	0x2
#define EC_POWER_BAD			0x4
