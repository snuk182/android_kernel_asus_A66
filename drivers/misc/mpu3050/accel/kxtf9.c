...
{	struct i2c_client *client = to_i2c_client(dev);
	struct kxtf9_mod_private_data *data = i2c_get_clientdata(client);
...
static ssize_t read_accel_raw(struct device *dev, struct device_attribute *devattr, char *buf)
{
    ...
    struct i2c_client *client = to_i2c_client(dev);
    struct kxtf9_mod_private_data *accel = i2c_get_clientdata(client);

    //printk("%s+\n", __func__);
    if(accel->gsensor_status)
    {
        res = inv_serial_read(client->adapter, accel->pdata->address,0x0F, 1, data);
        //printk("kxtf9 WHO AM I: %x\n",data[0]);

        res = inv_serial_read(client->adapter, accel->pdata->address,0x1B, 1, data);
        //printk("kxtf9 CTRL_REG1: 0x%x\n",data[0]);

        if((data[0] |= 0x80) == 0)
        {
            //printk("KXTF9 is standing by, change to OP \n");
            data[0] |= 0x80;
            inv_serial_single_write(client->adapter, accel->pdata->address,0x1B, data[0]);
        }

        res = inv_serial_read(client->adapter, accel->pdata->address,0x06, 6, data);
			   ...
			   const struct i2c_device_id *devid)
{
	...
	if (result) {
        ...
        private_data->attrs.attrs = kxtf9_attr;
