				 ...
				 struct v4l2_control *ctrl)
{
	...
	ctrlcmd.timeout_ms = 5000;//1000; avoid timeout
	ctrlcmd.vnode_id = pcam->vnode_id;
	ctrlcmd.queue_idx = pcam->server_queue_idx;
	ctrlcmd.config_ident = g_server_dev.config_info.config_dev_id[0];
...
//LiJen Fixed: removed pcam->pdev->name. It is Null. 
...
{
	int rc = -EINVAL;
	struct v4l2_control *ioctl_ptr = arg;
...
static int msm_open(struct file *f)
{
	...
	if (pcam->use_count == 1) {
		...
		if (rc < 0) {
			...
			//goto err_open_session; //ASUS_BSP LiJen "[A66][8M][NA][Others] add opensession error handle"
		}
...
//ASUS_BSP +++ LiJen "[A66][8M][NA][Others] add opensession error handle"
//err_open_session:
       //msm_cam_server_close_session(&g_server_dev, pcam); 
//ASUS_BSP --- LiJen "[A66][8M][NA][Others] add opensession error handle"

mctl_open_failed:
	printk("mctl_open_failed\n");
	if (pcam->use_count == 1) {
#ifdef CONFIG_MSM_MULTIMEDIA_USE_ION
...
msm_cam_server_open_session_failed:
	printk("msm_cam_server_open_session_failed\n");
	if (pcam->use_count == 1) {
		if (queue != NULL) {
			queue->queue_active = 0;
			msm_drain_eventq(&queue->eventData_q);
