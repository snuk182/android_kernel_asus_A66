...
#include <linux/wakelock.h>
static struct subsys_device *subsys = NULL;
...
// ASUS_BSP--- Wenli "Modify for modem restart"
	...
	pr_info("Restart requested for %s restart_level %d subsystem mask 0x%X\n",
...
// jack for modem ramdump capture +++++
static void save_ram_dump(struct subsys_device *sub_sys)
{
    pet_watchdog();
    sub_sys->desc->shutdown(sub_sys->desc);
    // Always savelog in this case
    sub_sys->desc->ramdump(1, sub_sys->desc);
...
static void subsystem_save_logs(void)
{
    ...
    sprintf(filename, "%s_Crash", subsys->desc->name);
...
static void subsystem_restart_fn(struct work_struct *work)
{
    ...
    printk_lcd("%s fatal error COUNTING DOWN %d !!", subsys->desc->name, warning_count);
